  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-white/8 bg-[#0a1f12] px-4">
      ...
      <div className="flex items-center gap-1.5">
        {/* Notifications */}
        <div className="relative">
          <NavIconBtn aria-label="Notifications" onClick={() => setNotifOpen((o) => !o)}>
            <Bell size={14} />
            <span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full border border-[#0a1f12] bg-ecomate-500" />
          </NavIconBtn>
        ...
        {/* Avatar → links to user's own profile */}
        <Link
          href={`/profile/${user.username}`}
          aria-label="View your profile"
          className="relative ml-1 h-8 w-8 shrink-0 overflow-hidden rounded-full border-2 border-ecomate-500 transition hover:border-ecomate-400"
        >
          {user.imageFile ? (
            <Image
              src={user.imageFile}
