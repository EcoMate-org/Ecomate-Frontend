...
function SigninForm() {
  ...
  const next = searchParams.get("next");
  const [serverError, setServerError] = useState<string | null>(null);
  const [needsVerification, setNeedsVerification] = useState<string | null>(null); // holds the email
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    if (!res.ok) {
      if (json.requiresVerification) {
        setNeedsVerification(json.email ?? values.email);
        return;
      }
      setServerError(json.error ?? "Unable to sign in. Please try again.");
      return;
  ...
  return (
    <div className="glass w-full max-w-md rounded-2xl p-8 shadow-xl">
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4" noValidate>
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <Link
              ...
              className="text-xs font-medium text-ecomate-600 hover:underline"
            >
              Forgot password?
            </Link>
          </div>
        ...
        {needsVerification && (
          <div className="rounded-md bg-yellow-50 px-3 py-2.5 text-sm text-yellow-800">
            <p>Please verify your email address before signing in.</p>
            {resendStatus === "sent" ? (
              <p className="mt-1 font-medium text-yellow-900">Verification email sent — check your inbox.</p>
            ) : (
              <button
                type="button"
                onClick={handleResend}
                disabled={resendStatus === "sending"}
                className="mt-1 font-medium text-ecomate-700 underline hover:text-ecomate-800 disabled:opacity-60"
              >
